    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-end gap-4">
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-finki-navy tracking-tight">Распоред на часови</h1>
        ...
        <div className="flex items-center gap-3 shrink-0">
          {slots && (
            <span className="text-sm text-gray-400">
              {slots.length} {slots.length === 1 ? "час" : "часови"}
            </span>
          )}
          <button
            onClick={copyLink}
          ...
          {schedule.loggedIn && (
            <Link
              href="/schedule"
              className="flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-semibold transition-colors shadow-sm bg-finki-navy text-white hover:bg-finki-mid"
            >
              ...
              {schedule.slotIds.size > 0 && (
                <span className="text-xs font-bold rounded-full min-w-[18px] h-[18px] px-1 flex items-center justify-center bg-white/20 text-white">
